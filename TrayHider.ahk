...
;
; NOTE: TrayHider runs elevated by default (UAC prompt on manual launch —
;       see ForceElevation in config.ini). Once "Start with Windows" is
;       enabled, the logon launch is silent.
;
; SAFETY:
...
MAX_TITLE     := 60
AUTOSTART_TASK := "TrayHider"   ; Task Scheduler task name. Registered at
                                ; /RL HIGHEST, so creating/deleting it needs
                                ; an elevated process — see the autostart
...
STRINGS := Map(
  "en", Map(
    ...
    "gRegFail",       "Some shortcuts failed to register (possibly conflicting with another app). The previous shortcut stays active for:`n{1}"),
  "id", Map(
    ...
    "gRegFail",       "Sebagian shortcut gagal didaftarkan (mungkin bentrok dengan aplikasi lain). Shortcut lama tetap aktif untuk:`n{1}"))
...
; it, this WILL click it. Kept in config.ini ([Settings]
; ClickToRefocusAfterHide, 1/0) rather than as a source constant so that
; users of the compiled .exe can turn it off too.
clickToRefocusAfterHide := true
...
; The tray icon MUST be established before the first RebuildTrayMenu and must
; never be left hidden — if the icon doesn't show, the entire tray menu is
; unreachable, which means a user with no hotkey set has no way to restore a
; hidden window and is forced to reboot. So this runs early, is wrapped so a
; bad icon source can't abort startup, and explicitly un-hides the icon.
...
;=============================================================================
LoadConfig() {
    global currentLang, currentKeys, forceElevation, clickToRefocusAfterHide
    ...
    clickToRefocusAfterHide := (IniRead(CONFIG_FILE, "Settings", "ClickToRefocusAfterHide", "1") != "0")
...
SaveConfig() {
    ...
    IniWrite(clickToRefocusAfterHide ? "1" : "0", CONFIG_FILE, "Settings", "ClickToRefocusAfterHide")
...
;
; This runs in three phases, and the separation is load-bearing:
;
;   Phase 1 releases EVERY changing binding before ANY registration happens.
;   Hotkey() is keyed by the key string, not by the action — so when two
;   functions SWAP keys, releasing lazily inside the registration loop would
;   switch off a key the previous iteration had just registered, silently
;   killing one of the two functions.
;
;   Phase 3 rolls back: an action whose new key failed to register gets its
;   previous key re-bound, so a failed change can never leave a function
;   without any hotkey at all (the old behaviour dropped the old binding
;   first and never restored it).
;
; Finally, only keys that are ACTUALLY bound are persisted to config.ini —
; the old code saved whatever was requested, so one failed apply wrote a
; known-broken combination to disk and re-surfaced it as a config error on
...
ApplyHotkeys(newKeys, startup := false) {
    ...
    ; Snapshot the previous bindings before anything changes (for rollback).
    oldKeys := Map()
    for name in actions
        oldKeys[name] := registered.Has(name) ? registered[name] : ""

    ; --- Phase 1: release every binding that is about to change -------------
    for name in actions {
        if (oldKeys[name] != "" && oldKeys[name] != newKeys[name]) {
            try Hotkey(oldKeys[name], "Off")
            registered[name] := ""
        }
    }

    ; --- Phase 2: register the new set ---------------------------------------
    failed := ""
    for name, cb in actions {
        if (registered.Has(name) && registered[name] = newKeys[name]) {
            ; Unchanged and still bound — but force it On in case it's
            ; currently suspended (the settings recorder turns hotkeys Off
            ; while open, and Save runs before that suspend is lifted).
            try Hotkey(newKeys[name], "On")
            continue
        }
        try {
    ...
    ; --- Phase 3: roll back any action left unbound ---------------------------
    ; Skip the rollback if another action now owns the old key (a swap where
    ; only one side failed) — re-binding it would steal that action's hotkey.
    for name, cb in actions {
        if (registered.Has(name) && registered[name] != "")
            continue
        if (oldKeys[name] = "")
            continue
        ...
        if ownedElsewhere
            continue
        try {
            Hotkey(oldKeys[name], MakeAction(cb), "On")
            registered[name] := oldKeys[name]
        }
    }

    ; Persist what is actually bound, not what was requested. On a failed
    ; apply this keeps the old (working) key in config.ini; on a startup
    ; failure the config value is left as-is so the user can still see and
    ; fix it via the Settings dialog, exactly as the error message says.
    for name in actions
        if (registered.Has(name) && registered[name] != "")
            currentKeys[name] := registered[name]
    SaveConfig()
...
RestoreLastWindow() {
    ...
    ; Keep popping until something is actually brought back. An entry whose
    ; window has died (or whose handle was recycled) is discarded silently by
    ; ShowAndActivate, which reports success — without this loop the hotkey
    ; would consume that dead entry and appear to do nothing, forcing the user
    ; to press it again once per stale entry.
    restored := false
    while (hiddenWindows.Length > 0) {
        entry := hiddenWindows.Pop()
        wasOurs := EntryStillOurs(entry)
        if ShowAndActivate(entry) {
            if wasOurs {
                restored := true
                break          ; a real window came back — stop here
            }
            continue           ; dead entry: dropped, keep looking
        }
        ; Still alive but the restore failed — put it back so the window
        ; stays reachable from the tray menu, and stop rather than silently
        ; unwinding the rest of the stack.
        hiddenWindows.Push(entry)
        break
    }
    if !restored && (hiddenWindows.Length = 0)
        TrayTip(L("empty"), L("emptyBody"))
    SavePersistence()
    RebuildTrayMenu()
}
...
;--- Restore a single entry, addressed by HWND (called from the tray menu) ----
; The menu handler deliberately captures the entry's HWND, not its index.
; Indices go stale while the menu is open: the watchdog prunes dead entries
; in the background, the list shifts, and a click on "2." would then restore
; a different window than the one the label promised. The HWND is stable for
; the entry's whole lifetime, so it is looked up at click time instead.
RestoreByHwnd(hwnd) {
    global hiddenWindows
    for idx, entry in hiddenWindows {
        if (entry.hwnd = hwnd) {
            hiddenWindows.RemoveAt(idx)
            if !ShowAndActivate(entry)
    ...
    ; Not found: the entry was pruned while the menu was open (window died).
    ; Nothing to restore — just bring the menu back in sync with reality.
    RebuildTrayMenu()
}
...
;--- Establish the tray icon and guarantee it is visible ----------------------
; If the icon isn't shown, the whole tray menu is unreachable — and for a user
; without a restore hotkey, that means a hidden window can only be recovered
; by rebooting. So every step is defensive: a failing icon source falls
; through to the next, and the icon is explicitly un-hidden at the end
; (A_IconHidden can end up set in some elevated/compiled launch paths).
EnsureTrayIconVisible() {
    done := false
    ; 1. Compiled .exe → the icon embedded in the exe itself.
    if A_IsCompiled {
        try {
    ...
    ; 2. Running as .ahk with icon.ico beside the script.
    if (!done && FileExist(A_ScriptDir "\icon.ico")) {
        try {
            TraySetIcon(A_ScriptDir "\icon.ico")
            done := true
        }
    }
    ; 3. Last-resort stock icon — this should essentially never fail, and if
    ;    even this throws we still fall through to the un-hide below so AHK's
    ;    own default icon stays visible.
    if !done {
        try TraySetIcon("shell32.dll", 172)
    }
    ; Whatever icon we ended up with, make sure it's actually shown.
    try A_IconHidden := false
}
...
RebuildTrayMenu() {
    ...
    if (hiddenWindows.Length = 0) {
        ...
        ; happen to share the same title. The handler captures the HWND, not
        ; the index — see RestoreByHwnd for why.
        for idx, entry in hiddenWindows
            tm.Add(idx ". " Truncate(entry.title), MakeRestoreHandler(entry.hwnd))
    }
    tm.Add()                                        ; separator
...
; Closure factory — locks in the HWND per menu item
MakeRestoreHandler(hwnd) => (*) => RestoreByHwnd(hwnd)
...
; for TrayHider, which runs elevated by default, it is also the ONLY way to
...
TaskSchedulerRoot() {
    ; The connected service object is cached, the QUERY is not. Connecting is
    ; an RPC to the Task Scheduler service and this runs from every
    ; RebuildTrayMenu (so, every hide); the GetTask call that actually answers
    ; "is autostart on?" still happens live every time, so a task deleted by
    ; hand in Task Scheduler is reflected immediately — no staleness.
    static svc := ""
    if (svc = "") {
    ...
    try
        ...
        ; The cached connection went stale (service restarted, COM object
        ; disconnected). Drop it and reconnect once before giving up, so a
        ; transient service hiccup doesn't disable autostart for the rest of
        ; the session.
        svc := ComObject("Schedule.Service")
        svc.Connect()
        return { svc: svc, folder: svc.GetFolder("\") }
...
; Queried live, on purpose. An earlier version cached the boolean because it
; shelled out to cmd.exe + schtasks.exe — tens of milliseconds on every
; tray-menu rebuild. Caching the ANSWER would now only buy staleness: the
; checkbox showing "enabled" after the user deleted the task by hand in Task
; Scheduler, until the next toggle happened to refresh it. What is cached
; instead is the service CONNECTION (see TaskSchedulerRoot), which removes
; the per-call RPC cost while keeping every answer current.
IsAutostartEnabled() {
    enabled := false
    try {
        ts := TaskSchedulerRoot()
        task := ts.folder.GetTask(AUTOSTART_TASK)   ; throws if the task doesn't exist
        ; Existence alone is not enough: a task left over from a previous
        ; install (or an older version at a different path) would otherwise
        ; light up the checkbox even though it wouldn't launch THIS build.
        ; Confirm the task's action actually points at the executable we're
        ; running from before reporting it as enabled.
        enabled := TaskPointsHere(task)
    }
    return enabled
}

; True if the task's first exec action targets the current executable. Any
; mismatch (old path, moved install, wrong action) counts as "not enabled",
; so the user re-enabling it writes a correct, current task.
TaskPointsHere(task) {
    want := A_IsCompiled ? A_ScriptFullPath : A_AhkPath
    try {
        for action in task.Definition.Actions {
            if (action.Type = 0) {   ; TASK_ACTION_EXEC
                have := action.Path
                ; Normalise surrounding quotes/whitespace before comparing.
                have := Trim(have, ' "')
                return (have = want)
            }
        }
...
ToggleAutostart(*) {
    ...
    RebuildTrayMenu()               ; refresh the checkmark (queries live)
}
...
; Called when registering or deleting the autostart task fails. Normally
; rare, since TrayHider is already elevated by this point — it mainly covers
; the case where the user cancelled the startup UAC prompt. Offers to
; relaunch as Administrator and automatically retries the SAME toggle action
; once elevated.
...
SetLanguage(lang) {
    ...
    if settingsGui {              ; close the settings window so it doesn't
        ...
        settingsGui := 0
        ; The settings window suspends the global hotkeys while open; since we
        ; destroyed it directly instead of via CloseSettings, resume them here
        ; too — otherwise the hotkeys would stay dead until the next restart.
...
ShowSettingsGui() {
    ...
    ; Suspend the global hotkeys while the settings window is open. Otherwise,
    ; the moment the user presses (say) Alt+F1 into the recorder control, the
    ; still-active global Alt+F1 fires too and hides the settings window (or a
    ; window behind it). Suspend makes the keypress reach ONLY the Hotkey
    ; control. Hotkeys are resumed in CloseSettings, whichever way it closes.
    ...
    SaveSettings(*) {
        ...
        ; Apply the new keys, THEN close (which resumes hotkeys). ApplyHotkeys
        ; re-registers everything, so the resume below picks up the new set.
    ...
    CloseSettings() {
        ...
        ; Always re-enable the global hotkeys, no matter how the window closed
        ; (Save, Cancel, the X button, or a language switch destroying it).
        SuspendGlobalHotkeys(false)
    }
}

;--- Turn every registered global hotkey off (true) or back on (false) --------
; Used only around the settings recorder. Iterates the same key set
; ApplyHotkeys tracks, so it stays correct even after the user has remapped.
SuspendGlobalHotkeys(suspend) {
    global currentKeys
    for , keyStr in currentKeys {
        if (keyStr = "")
            continue
        try Hotkey(keyStr, suspend ? "Off" : "On")
    }
}
...
HandleExit(reason, code) {
    RestoreAllWindows()   ; also deletes the persistence file once the list
                          ; is empty; entries that FAIL to restore stay in it,
                          ; so the next launch can still recover them
...
SavePersistence() {
    ...
    try {
        ...
        ; No hidden windows → no file at all. This is what actually clears
        ; the persistence state on a clean exit, and it lets RecoverFromCrash
        ; short-circuit on FileExist instead of opening and parsing a
        ; header-only leftover on every launch.
        if (hiddenWindows.Length = 0)
            return
        ; A version header lets the reader tell the current field order apart
        ; from the older hwnd|pid|title|exStyle layout. Without it, an old
        ; file whose title happens to be numeric ("123") would be parsed as an
        ; exStyle and silently applied to the window — a wrong style with no
        ; error.
        ...
        for entry in hiddenWindows
            ...
            ; With title last, StrSplit's MaxParts lets the remainder absorb
            ; any embedded pipes harmlessly. Putting exStyle after title would
            ; make Integer() parse a chunk of the title and throw.
