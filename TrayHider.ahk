...
HideActiveWindow() {
    ...
    ; SW_HIDE alone is not enough for some apps: their own message loop can
    ; call ShowWindow again and pop the taskbar button back. Marking the
    ; window as a tool window (and clearing the app-window flag) tells the
    ; shell not to give it a taskbar button or an Alt+Tab entry at all, which
    ; survives the app re-showing itself.
    SetToolWindowStyle(hwnd, true)
...
ShowAndActivate(entry) {
    try {
        ; Undo the tool-window marking BEFORE showing, so the shell creates a
        ; normal taskbar button again as the window becomes visible.
        SetToolWindowStyle(entry.hwnd, false)
        ; Put back the exact extended style the window had before we touched
        ; it — safer than assuming it was a plain default.
...
WatchdogTick() {
    global hiddenWindows
    for entry in hiddenWindows {
        ; Skip windows whose process has died — RecoverFromCrash/Exit handle those
    ...
    try {
        Loop Parse, FileRead(PERSIST_FILE, "UTF-8"), "`n", "`r" {
            ...
            if WinExist("ahk_id " hwnd) {
                ...
                if (curPid = pid) {
                    try {
                        ; A crashed session left WS_EX_TOOLWINDOW applied, so
                        ; the window would come back with no taskbar button.
                        ; The exact original style isn't recorded on disk, but
                        ; clearing the tool-window flag is what actually matters.
                        SetToolWindowStyle(hwnd, false)
                        WinShow(hwnd)
