...
- The script legitimately calls low-level Windows APIs (`WinHide`, the Task Scheduler COM API for the autostart toggle, global keyboard hooks) — the exact same APIs many AV heuristics associate with spyware/keyloggers, even though here they're used for a visible, user-triggered tray utility.
...
- **Start with Windows** toggle (checkbox in the tray menu — registers a Task Scheduler entry so the app launches silently at logon)
...
- **TrayHider runs elevated by default** (a UAC prompt on manual launch). This lets it hide windows that are themselves running as administrator, and — once "Start with Windows" is enabled — lets the logon launch happen silently with no prompt. If you only ever hide ordinary windows (a browser, Notepad, a game) and don't want the prompt, set `ForceElevation=0` in `config.ini`; note that with elevation off, autostart can no longer launch silently.
...
2. Run it — **the installer itself needs no admin rights**: it installs to your own user profile (`%LocalAppData%\TrayHider`), not Program Files, so setup never shows a UAC prompt.
3. When TrayHider first launches it *will* show a UAC prompt, because the app runs elevated by default (see Known limitations for why, and how to turn that off).
4. Enable "Start with Windows" from the tray icon whenever you want autostart — it's intentionally left off by a fresh install so nothing runs at logon until you ask for it.
...
- Run as Administrator (shown only if not already elevated — e.g. if you declined the startup prompt)
...
All settings live in `config.ini`, created next to the executable on first run. Hotkeys and language are also editable from the tray menu; the two options below are file-only:

| Setting | Default | Effect |
|---|---|---|
| `ForceElevation` | `1` | `1` runs elevated (UAC prompt on manual launch, can hide elevated windows, silent autostart). `0` runs normally with no prompt, but can't hide elevated windows and autostart can't launch silently |
| `ClickToRefocusAfterHide` | `1` | After hiding a window, simulates a click at the cursor so the window underneath takes focus. Set `0` if that click ever triggers something in the window behind |
...
| Runs elevated at startup | On by default so it can hide elevated windows and support silent autostart. A single UAC prompt on manual launch. Turn it off with `ForceElevation=0` in `config.ini` if you don't need it |
| Task Scheduler entry | Only when you enable "Start with Windows"; created via the Task Scheduler COM API at `HIGHEST` run level so the logon launch is silent (no per-boot UAC). Fully reversible from the same checkbox, and removed on uninstall |
...
- Script ini memang memanggil API Windows level-rendah (`WinHide`, Task Scheduler COM API untuk fitur autostart, global keyboard hook) — API yang sama yang sering dikaitkan heuristik antivirus dengan spyware/keylogger, walau di sini dipakai untuk utility tray yang terlihat dan dipicu user sendiri.
...
- Toggle **Mulai saat Windows boot** (checkbox di menu tray — mendaftarkan entri Task Scheduler agar app jalan senyap saat logon)
...
- **TrayHider berjalan elevated secara default** (muncul prompt UAC saat dijalankan manual). Ini agar bisa menyembunyikan jendela yang berjalan sebagai administrator, dan — setelah "Mulai saat Windows boot" diaktifkan — agar peluncuran saat logon berlangsung senyap tanpa prompt. Jika Anda hanya menyembunyikan jendela biasa (browser, Notepad, game) dan tidak ingin prompt-nya, set `ForceElevation=0` di `config.ini`; dengan elevasi mati, autostart tidak bisa lagi jalan senyap.
- Ringan: ~1MB hasil kompilasi, RAM idle beberapa MB saja

## 🚀 Opsi A — Installer (disarankan)

1. Buka [**Releases**](../../releases) dan unduh **`TrayHider-Setup.exe`**.
2. Jalankan — **installer-nya sendiri tidak butuh hak admin**: terpasang ke folder profil Anda (`%LocalAppData%\TrayHider`), bukan Program Files, jadi proses instalasi tidak memunculkan prompt UAC.
3. Saat TrayHider pertama kali jalan, UAC *akan* muncul, karena app berjalan elevated secara default (lihat Batasan untuk alasannya dan cara mematikannya).
4. Aktifkan "Mulai saat Windows boot" dari ikon tray kapan pun Anda mau autostart — sengaja dibiarkan mati oleh instalasi baru agar tidak ada yang jalan saat logon sampai Anda memintanya.
...
Klik kanan ikon tray untuk: daftar jendela tersembunyi, Restore All, Pengaturan Shortcut, toggle Mulai saat boot, Jalankan sebagai Administrator (muncul hanya bila belum elevated), ganti bahasa, About, dan Exit (otomatis memulihkan semua jendela terlebih dahulu).
